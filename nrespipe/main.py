...
def stack_nres_calibrations():
    ...
    parser.add_argument('--start', required=True, help='Starting datetime to stack. Format should be %YYYY-%mm-%ddT%HH:%MM:%ss')
    parser.add_argument('--end', required=True, help='Ending datetime to stack. Format should be %YYYY-%mm-%ddT%HH:%MM:%ss')
    parser.add_argument('--nres-instrument', dest='nres_instrument', required=True,
                        help='NRES instrument name (e.g. nres01)')
    args = parser.parse_args()

    date_format = '%yyyy-%mm-%ddT%HH:%MM:%ss'
    date_range = [datetime.datetime.strptime(args.start, date_format), datetime.datetime.strptime(args.end, date_format)]
    tasks.make_stacked_calibrations.delay(args.site, args.camera, args.calibration_type, date_range,
