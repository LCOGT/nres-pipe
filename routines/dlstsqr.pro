function dlstsqr,dat,funs,wt,nfun,rms,chisq,outp,type
;
...
; matrix
  ...
  nx=s(1)
  wte=double(rebin(wt,nx,nfun))
  datw=reform(double(dat),nx,1)
  datw=rebin(datw,nx,nfun)*wte
  funw=double(funs)*wte
...
; Make fit function, rms
  ...
  chisq=total(dif2(s)*wt2(s))/ndegfree
