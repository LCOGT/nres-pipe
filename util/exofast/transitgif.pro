...
pro transitgif, pname, band=band, cadence=cadence, pars=pars, ymin=ymin,$
                ...
                update=update, bak=bak, maxstar=maxstar
...
blue = 256L^2*bak[2] + 256L*bak[1] + bak[0]
...
for i=0, nframes-1 do begin
   ...
   ;; plot the model
   plot, hours, flux, xstyle=9,ystyle=5,position=[0,0.1,1,0.4],$
         yrange=[ymin,1.002d],xrange=[xmin,xmax],color=black,$
         xtitle=TeXtoIDL('Time - T_C (hours)'),title=title,background=blue
   
   ;; overplot the planet at the current frame
   plotsym, 0, /fill, color=256^2*bplanet + 256*gplanet + rplanet
   oplot, [hours[i]], [flux[i]], psym=8,symsize=2
