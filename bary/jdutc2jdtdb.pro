...
if n_elements(tbase) gt 1 then $
   for i=0, n_elements(jd_tt)-1 do $
      tmp[i] = TDB2TDT(jd_tt[i], tbase=tbase[i]) $ 
else tmp = TDB2TDT(jd_tt, tbase=tbase)
;tmp2 = TDB2TDT(jd_tt, tbase=tbase)
clock_corr += tmp

;if n_elements(tbase) gt 1 then plot, tmp-tmp2
;stop
...
;   if n_elements(clock_corr) ge 268 then ndx = 267 $
;   else ndx = 0
;endif
;print, clock_corr[ndx],jd_utc[ndx], tbase[ndx], jd_tdb[ndx], jd_tt[ndx],TDB2TDT(jd_tt[ndx], tbase=tbase[ndx]),clock_corr2[ndx],format='(d,d,d,d,d,d,d)'
;stop
