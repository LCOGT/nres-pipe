pro get_calib_1,stype,filename,cdat,chdr,gerr,inst=inst
...
; line with navg=1.
; If keyword inst is set, then its value replaces that of environment variable
; NRESINST.
...
stds_rd_1,types,fnames,navgs,sites,cameras,jdates,flags,stdhdr,inst=inst
...
if(strlen(stypeu) gt 4) then begin
  stypeu=strmid(stypeu,0,4)
  types=strmid(types,0,4)
endif
...
if(keyword_set(inst)) then begin
  path=nresroot+inst+'/reduced/'+filename
endif else begin
